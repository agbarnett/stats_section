...
and \(H \in R ^{N \times g}\) \citep{aggarwal2012mining}. The number of
subgroups of common topic inferred from the data is represented by
\(g\), leading to the approximation: \begin{equation}
\label{eq:approxPNMF}
 P \approx WH^{T}.
\end{equation}
...
maximum coefficient value in the corresponding row of \(H\).

For our two datasets, we applied NMF varying \(g\) from 1 to 50.
Clustering solutions were assessed using silhouette score,
within-cluster dispersion and between-cluster dispersion.
...
sentences by the number of words common to each sentence (intersection),
...
\subsection{Included studies}\label{included-studies}}

\hypertarget{plos-one}{%
\subsubsection{PLOS ONE}\label{plos-one}}
...
Searching with the \emph{PLOS ONE} API returned 131,847 results (DOIs)
based on chosen search terms (\autoref{fig:plos-flowchart}). After
partial matching, 111,731 (85\%) statistical methods sections were
identified. In the final sample, 95,518 (85\%) DOIs returned an exact
match against common section headings: 64,133 for `statistical
analysis', 13,380 for `statistical analyses' and 13,627 for `data
analysis'. For our random sample of DOIs that did not meet the partial
matching criteria, initial search terms appeared in Introduction
...
\caption{\label{tab:unnamed-chunk-7}Example boilerplate text from PLOS ONE dataset}
\centering
\begin{tabular}[t]{l|l|r}
\hline
Topic & Statistical methods text & Number of matching studies\\
\hline
...
Finally, targeted searches for the term `statistician' revealed
