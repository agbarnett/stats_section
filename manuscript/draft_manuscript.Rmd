...
Classifying statistical methods sections into two clusters showed a general split between means-based testing and model-based analysis. Increasing the number of clusters decreased cluster quality based on global goodness-of-fit measures (Supplementary Figure - Silhouette + between- & within-cluster dispersions). This indicated that the data comprised one large, heterogeneous cluster and multiple smaller clusters. To examine the effect of increasing the number of clusters on cluster content, we examined themes present among smaller clusters under five-topic (\@ref(fig:plot-5-topics)) and ten-topic (\@ref(fig:plot-10-topics) solutions. Examples of content among top matches for each topic are presented in Table X.

Topic 1 had the smallest number of unique terms under both five (n = 12,531; 54,780 unique terms) and ten (n = 3,775; 24,686 unique terms) topic solutions. Sections assigned to Topic 1 focused on the use of Student's t-test followed by statements of statistical significance. Under the ten topic solution, Topic 1 was split into two clusters based on the use of 5% (Topic 1: n = 3,775) versus multiple thresholds for declaring statistical significance (Topic 9: n = 6,104).

Two topics concentrated on statistical software representing 26% and 17% of all statistical methods sections under five and ten topic solutions. Review of top matching articles assigned to Topic 3 (Prism GraphPad) revealed evidence of boilerplate text, with many omitting details about statistical methods used. For sections citing the use of SPSS, increasing the number of clusters appeared to isolate references to software only versus articles that provided further details on statistical methods used.

Topic 4 was characterised by the use descriptive statistics followed by between-group testing including post-hoc multiple comparisons. Descriptive statistics were commonly reported as means with standard deviations or standard errors. Related topics focused on analysis of experimental designs (Topic 10: n = 4,664) and two-group analyses only (Topic 6: n = 4,746).

Remaining topics reflected applications of meta-analysis (Topic 7: n = 5,165), regression/survival analysis (Topic 2: n = 19,478) and larger sections that combined data processing and analysis (Topic 8: n = 38,183) [TODO: more work needed on this]

and  Larger clusters focused on more advanced statistical methods
