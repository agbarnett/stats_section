...
for (t in 1:nrow(examples_in_sentences)){
  ...
  dat.sentences = lapply(1:nrow(this_topic), function(i) str_split(this_topic[i,]$text_data_clean,"\\. ",simplify=F) %>% unlist() %>% 
                           tibble(text_data_clean_s=.) %>% add_column(doi = this_topic[i,] %>% pull(number)))
    ...
    this_paper = dat.sentences[[k]] %>%
      ...
      if(nrow(this_paper_similar)>0){
        score = stringsim(a = examples_in_sentences$sentence[t], # compares example sentence with every sentence in the paper
                          ...
                          method = c("jaccard"),
                          useBytes = FALSE)
        f = data.frame(number = this_topic$number[k], score=score, row=t)
