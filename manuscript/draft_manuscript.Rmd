...
We compiled an extensive list of common methods and statistical terminology to standardise descriptions of statistical analyses undertaken. An initial list was compiled by calculating individual word frequencies in the data and identifying relevant terms. Additional terms were added based on index searches of statistical textbooks [TODO need to mention this here? If so, add references] and spelling checks (e.g. ‘fischer’ instead of ‘fisher’). Multiple searches were then performed to identify possible variants of list terms, which were replaced with a standardised spelling. For example, instances ‘chi square’ and ‘chi squared’ were replaced with ‘chi-square’. Other variants of terminology considered plurals (e.g. chi squares), combinations of characters and numbers (e.g. chi 2) and combined text (e.g. chisquare).

As a final step, common stop words including pronouns, contractions and selected prepositions were removed from the data. We chose to retain selected stop words that, if excluded, may have changed the context of statistical method(s) being described (e.g. ‘between’, ‘against’).
